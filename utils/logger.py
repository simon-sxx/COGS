...
from utils.utils import omegaconf2dict, omegaconf2list
...
class MyLogger():
    ...
    def log_metrics(self, metrics_dict: Dict[str, float], figure_dict: Dict={}, iters=0):
        for logger_name in self.logger_dict.keys():
            if logger_name == 'csvlogger':
                ...
                for k in metrics_dict.keys():
                    if isinstance(metrics_dict[k], (int, float)):
                        self.logger_dict[logger_name].add_scalar(
                            k, metrics_dict[k], iters)
                    else:
                        raise NotImplementedError
        for k in figure_dict.keys():
            self.log_figures(name=k,
                             figure=figure_dict[k],
                             iters=iters)
    ...
    def log_figures(self, figure, name="figure.png", iters=None, exclude_logger=[]):
        ...
        if iters is None:
            save_path = opj(self.log_dir, "figure.png")
        else:
    ...
    def log_npz(self, data: dict, name="data.npz", iters=None):
        if iters is None:
            save_path = opj(self.log_dir, name)
        else:
            save_path = opj(self.log_dir, f"iter_{iters:d}", name)
        os.makedirs(opd(save_path), exist_ok=True)
        np.savez(save_path, **data)
        
    def log_txt(self, text: str, name="data.log", iters=None, exclude_logger=[], write_type="w"):
        for logger_name in self.logger_dict.keys():
            if logger_name == 'tblogger':
                if logger_name not in exclude_logger:
                    self.logger_dict[logger_name].add_text(name, text, global_step=iters)
        if iters is None:
            save_path = opj(self.log_dir, name)
        else:
            save_path = opj(self.log_dir, f"iter_{iters:d}", name)

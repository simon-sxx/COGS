...
def train_valid_target_eval_names(args):
    ...
    for i in range(args.domain_classes+1):
...
def eval_ood_performance(preds, labels, name=''):
    scores = {}
    total_correct = 0
    total_samples = 0
...
def plot_feature_sel(prob, bs=None, class_names=None, figsize=None, vmin=None, vmax=None, show_text=True, cmap="magma"):
    """
    ...
    num_classes = prob.shape[0]
    if len(prob.shape) == 1:
        prob = prob.reshape(bs, prob.shape[0]//bs)
    if class_names is None or type(class_names) != list:
        class_names = [str(i+1) for i in range(num_classes)]
    ...
    for i, j in itertools.product(range(prob.shape[0]), range(prob.shape[1])):
        ...
        if show_text:
            plt.text(j, i, format(prob[i, j], ".2f") if prob[i, j] != 0 else ".",
                    horizontalalignment="center", color=color, size=7)
    ...
    if bs is None:
        ...
        plt.ylabel("Variable_p1")
        plt.xlabel("Variable_p2")
